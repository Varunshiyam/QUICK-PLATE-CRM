...
const Orders = () => {
    ...
    const fetchOrders = async () => {
      try {
        if (isMockMode) {
          ...
          setOrders(MOCK_ORDERS.slice(start, end));
          setTotalPages(Math.ceil(MOCK_ORDERS.length / PAGE_SIZE));
          localStorage.setItem(
            'quickplate_orders',
            JSON.stringify(MOCK_ORDERS.slice(start, end))
          );
        } else {
          try {
            ...
            const res = await axios.get(
              `${API_BASE_URL}/services/apexrest/customer/orders`,
              {
                params: {
                  idToken,
                  page,
            ...
            if (res.data && res.data.orders && res.data.orders.length > 0) {
              const mappedData = res.data.orders.map(item => {
                const restaurantName = item.restaurantName || 'Quick Plate Order';
                return {
  ...
  const pastOrders = orders.filter(order => order.status === 'DELIVERED' || order.status === 'Delivered' || order.status === 'REFUNDED');
